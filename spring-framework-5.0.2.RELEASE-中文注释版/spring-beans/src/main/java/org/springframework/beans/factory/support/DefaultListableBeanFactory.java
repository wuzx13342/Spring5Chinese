...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
		...
		for (String beanName : beanNames) {
			...
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				...
				if (isFactoryBean(beanName)) {
					...
					if (isEagerInit) {
						//TODO 调用getBean方法，触发容器对Bean实例化和依赖注入过程
						getBean(beanName);
					}
				}
				else {
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		//TODO 至此 已完成初始化IOC容器的所有工作 IOC容器建立了整个Bean的配置信息，Bean定义信息是IOC控制反转的基础，
		// 接下来可以进行依赖注入 依赖注入的入口是应用程序第一次通过BeanFactory.getBean()方法触发
		// 注意：还有一种lazy的在IOC初始化的时候调用getBean进行预注入了 具体可以看refresh().finishBeanFactoryInitialization方法（DI）
	}
